...
|        파일명         | 역할            | 설명                                                                          |
| :-------------------: | :-------------- | :---------------------------------------------------------------------------- |
|     **vk_01.py**      | 일정 수집       | 최근 5개 시즌의 경기 일정과 기본 결과를 수집합니다.                           |
|     **vk_02.py**      | 상세 크롤링     | 경기별 선수 기록, 팀 기록 등 세부 데이터를 수집합니다. (스마트 업데이트 적용) |
|     **vk_03.py**      | 데이터 전처리   | JSON 형태의 원본 데이터를 분석 가능한 CSV(Tabular) 형태로 가공합니다.         |
|     **vk_04.py**      | 모델 학습       | ELO 및 파생 변수를 생성하고, 논리 제약이 적용된 AI 모델을 학습시킵니다.       |
|     **vk_05.py**      | 승부 예측       | 학습된 모델을 불러와 오늘의 경기 결과를 예측하고 분석 리포트를 출력합니다.    |
